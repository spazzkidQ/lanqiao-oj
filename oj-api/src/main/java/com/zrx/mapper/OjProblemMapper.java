...
public interface OjProblemMapper extends BaseMapper<OjProblem> {
    //获取提交总数（所有未删除的提交）
    @Select("SELECT COUNT(*) FROM oj_problem_submit WHERE question_id = #{problemId} AND is_delete = 0")
    Integer getSubmitNum(Long problemId);

    //获取通过数（只统计判题成功的提交）
    @Select("SELECT COUNT(*) FROM oj_problem_submit WHERE question_id = #{problemId} AND status = 2 AND is_delete = 0")
    ...
    int updateSubmitAndAcceptedNum(Long problemId, Integer submitNum, Integer acceptedNum);
